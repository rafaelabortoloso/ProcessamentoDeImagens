...
export const toAnd = (firstImage: ImageData, secondImage: ImageData) => {
  ...
  for (let i = 0; i < result.data.length; i += 4) {
    result.data[i] = Math.min(result.data[i], secondImage?.data[i] || 0)
    result.data[i + 1] = Math.min(result.data[i + 1], secondImage?.data[i + 1] || 0)
    result.data[i + 2] = Math.min(result.data[i + 2], secondImage?.data[i + 2] || 0)
  }
...
export const toOr = (firstImage: ImageData, secondImage: ImageData) => {
  ...
  for (let i = 0; i < result.data.length; i += 4) {
    result.data[i] = Math.max(result.data[i], secondImage?.data[i] || 0)
    result.data[i + 1] = Math.max(result.data[i + 1], secondImage?.data[i + 1] || 0)
    result.data[i + 2] = Math.max(result.data[i + 2], secondImage?.data[i + 2] || 0)
  }
...
export const toXor = (firstImage: ImageData, secondImage: ImageData) => {
  ...
  for (let i = 0; i < result.data.length; i += 4) {
    result.data[i] = result.data[i] !== secondImage?.data[i] ? 255 : 0
    result.data[i + 1] = result.data[i + 1] !== secondImage?.data[i + 1] ? 255 : 0
    result.data[i + 2] = result.data[i + 2] !== secondImage?.data[i + 2] ? 255 : 0
  }
